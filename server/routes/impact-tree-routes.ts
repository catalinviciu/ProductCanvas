...
// Schema for optimistic batch updates
...
router.post('/api/impact-trees/batch-update', isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const validatedData = batchUpdateSchema.parse(req.body);
    
    console.log('Batch update request:', { userId, updateCount: validatedData.updates.length });
    
    const results = [];
    
    // Process each update
    for (const update of validatedData.updates) {
      const { nodeId, treeId, position, content, structure } = update.data;
      
      try {
        // Prepare update data based on type
        const updateData: any = {};
        
        if (update.type === 'position' && position) {
          updateData.position = position;
        }
        
        if (update.type === 'content' && content) {
          Object.assign(updateData, content);
        }
        
        if (update.type === 'structure' && structure) {
          Object.assign(updateData, structure);
        }
        
        // Update the node
        const updatedNode = await treeService.updateNode(treeId, nodeId, userId, updateData);
