...
const TreeNodeComponent = memo(function TreeNode({
  ...
  const parentNode = useMemo(() => {
    ...
    if (process.env.NODE_ENV === "development" && allNodes) {
      console.debug(
        "TreeNode: Using prop-based allNodes lookup (less efficient) - consider using TreeContext",
      );
    }
    ...
    (e: React.DragEvent) => {
      ...
      if (draggedNodeId && draggedNodeId !== node.id) {
        if (onReattach) {
          ...
          console.warn(
            "onReattach prop is not provided - drag and drop reattachment will not work",
          );
          setDraggedOverNodeId(null);
        }
      } else {
  ...
  const handleAddChildContextMenu = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  const handleSaveEdit = useCallback(() => {
    console.log("Save edit clicked for node:", node.id, {
      editTitle,
      editDescription,
    });
    setIsEditing(false);
  ...
  const nodeClassName = useMemo(() => {
    ...
    if (isDropTarget && draggedNode) {
      stateClasses.push(
        "drop-target animate-pulse border-2 border-blue-400 bg-blue-50 shadow-xl",
      );
    }

    if (isDraggedOver) {
      stateClasses.push(
        "dragged-over bg-yellow-50 border-yellow-300 border-2 shadow-lg transform scale-105",
      );
    }
  ...
  return (
    ...
    >
      <div className="modern-card node-created relative group flex flex-col">
        ...
        <div className="flex items-center justify-between mb-2 flex-shrink-0">
          ...
          <div className="flex items-center space-x-1">
            {/* Status indicator for all node types except metric - always visible */}
            {node.type !== "metric" && onStatusChange && (
        ...
        <div className="flex-1 flex flex-col min-h-0">
          {isEditing ? (
              ...
              <div className="relative">
                ...
                {isTextLimitReached && (
                  <div className="text-limit-indicator">
                    ...
                    <span className="text-xs text-orange-600">3 rows max</span>
                  </div>
                )}
              </div>
            ...
            <div
              className="content-area-compact flex-1 overflow-hidden"
              onDoubleClick={handleDoubleClick}
            >
              <h3 className="node-title-compact line-clamp-3">
                {node.title.startsWith("New ") ? (
                  <span className="text-gray-400 italic">
                    {getNodePlaceholder(node.type)}
                  </span>
                ) : (
                  node.title
                )}
      ...
      {!isEditing && (
        ...
        >
          <div
            className={`children-badge-external hover:scale-110 transition-transform duration-200 ${
              collapseState.hasChildren
                ? node.isCollapsed
                  ? "collapsed"
                  : "expanded"
          ...
          >
            <span className="children-count-external">
          ...
          {collapseState.hasChildren && (
            <div className="status-icon-external hover:scale-110 transition-transform duration-200">
              <i
                className={`fas ${node.isCollapsed ? "fa-eye-slash" : "fa-eye"} text-xs ${
